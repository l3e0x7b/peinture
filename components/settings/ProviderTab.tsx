...
interface ProviderTabProps {
  ...
  setNewProviderUrl: (v: string) => void;
  newProviderToken: string;
  setNewProviderToken: (v: string) => void;
  fetchStatus: string;
...
export const ProviderTab: React.FC<ProviderTabProps> = (props) => {
  ...
  const renderProviderPanel = (
    ...
    <div key={id} className="border-b border-white/5 last:border-0">
      <div className="flex items-center w-full group">
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-2 h-2 rounded-full ${dotColorClass} ring-4 ring-white/[0.02] group-hover:ring-white/[0.05] transition-all`}
            />
            <span
              className={`text-sm font-medium transition-colors ${openProvider === id ? "text-white" : "text-white/60 group-hover:text-white"}`}
            >
              {title}
            </span>
          </div>
          <div
            className={`text-white/40 transition-transform duration-300 mr-2 ${openProvider === id ? "rotate-180 text-white/80" : "rotate-0 group-hover:text-white/60"}`}
          >
      ...
      <div
        className={`grid transition-[grid-template-rows] duration-300 ease-out ${openProvider === id ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
        <div className="overflow-hidden">
          <div className="p-2 space-y-4 mb-2">{children}</div>
        </div>
      </div>
    </div>
  );
      ...
      {statsObj.total > 1 && (
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-green-500/10 border border-green-500/10 rounded-xl p-2.5 text-center">
            <div className="text-[10px] text-green-400/60 uppercase tracking-wider mb-0.5">
              {t.tokenActive}
            </div>
  ...
  const showCustomProviders =
    props.serviceMode === "server" || props.serviceMode === "hydration";
        ...
        <>
          {renderProviderPanel(
            ...
            renderTokenInput(
              "hf",
              props.token,
              (v) => props.updateToken("huggingface", v),
              props.stats,
              "hf_...,hf_...",
              t.hfTokenHelp,
              t.hfTokenLink,
          ...
          {renderProviderPanel(
            ...
            renderTokenInput(
              "ms",
              props.msToken,
              (v) => props.updateToken("modelscope", v),
              props.msStats,
              "ms-...,ms-...",
              t.msTokenHelp,
              t.msTokenLink,
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={cp.name}
                    onChange={(e) =>
                      props.handleUpdateCustomProvider(cp.id, {
                        name: e.target.value,
                      })
                    }
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-medium text-white/60">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50 font-mono"
                    />
                    <button
                      onClick={() => props.handleRefreshCustomModels(cp.id)}
                      disabled={props.refreshingProviders[cp.id]}
                      className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all flex items-center gap-1.5 ${props.refreshingProviders[cp.id] ? "bg-white/5 text-white/40 border-white/5 cursor-not-allowed" : "bg-white/10 text-white/80 border-white/10 hover:bg-white/20"}`}
                      title={t.get_models || "Update Models"}
                    >
                      {props.refreshingProviders[cp.id] ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative w-full">
                    <input
                      type={showTokens[cp.id] ? "text" : "password"}
                      value={cp.token || ""}
                      onChange={(e) =>
                        props.handleUpdateCustomProvider(cp.id, {
                      ...
                      className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50 font-mono pr-8"
                    />
                    <button
                      type="button"
                ...
                <div className="flex items-center justify-between">
                  {props.refreshErrorProviders[cp.id] ? (
                    ...
                    <div
                      className={`text-xs transition-colors duration-300 flex items-center gap-1.5 ${props.refreshSuccessProviders[cp.id] ? "text-green-400 font-medium" : "text-white/40"}`}
                    >
                      {props.refreshSuccessProviders[cp.id] && (
                        <Check className="w-3 h-3" />
                      )}
                      {t.models_count.replace(
                        "{count}",
                        (
                  ...
                  <button
                    onClick={() => props.handleDeleteCustomProvider(cp.id)}
            ...
            renderProviderPanel(
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-medium text-white/60">
                    {t.provider_name}{" "}
                    <span className="text-white/30">({t.seedOptional})</span>
                  </label>
                  <input
                    type="text"
                    value={props.newProviderName}
                    onChange={(e) => props.setNewProviderName(e.target.value)}
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      value={props.newProviderUrl}
                      onChange={(e) => props.setNewProviderUrl(e.target.value)}
                      className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50 font-mono"
                      placeholder="https://example.com/api"
                    />
                    <button
                      onClick={props.handleFetchCustomModels}
                      disabled={
                        !props.newProviderUrl || props.fetchStatus === "loading"
                      }
                      className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all flex items-center gap-1.5 ${props.fetchStatus === "success" ? "bg-green-500/20 text-green-400 border-green-500/30" : props.fetchStatus === "failed" ? "bg-red-500/20 text-red-400 border-red-500/30" : "bg-white/10 text-white/80 border-white/10 hover:bg-white/20"}`}
                    >
                      ...
                      {props.fetchStatus === "loading"
                        ? t.fetch_status_loading
                        : props.fetchStatus === "success"
                          ? t.fetch_status_success
                          : props.fetchStatus === "failed"
                            ? t.fetch_status_failed
                            : t.get_models}
                    </button>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative w-full">
                    <input
                      ...
                      value={props.newProviderToken}
                      onChange={(e) =>
                        props.setNewProviderToken(e.target.value)
                      }
                      className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white focus:outline-0 focus:border-purple-500/50 font-mono pr-8"
                    />
                    <button
                ...
                {props.fetchedModels && (
                  <div className="p-3 bg-white/5 rounded-lg text-xs text-green-400 border border-green-500/20 flex items-center gap-2">
                    ...
                    {t.models_count.replace(
                      "{count}",
                      (
                        (props.fetchedModels.generate?.length || 0) +
                        (props.fetchedModels.edit?.length || 0) +
                        (props.fetchedModels.video?.length || 0) +
                        (props.fetchedModels.text?.length || 0) +
                        (props.fetchedModels.upscaler?.length || 0)
                      ).toString(),
                    )}
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <button
                    onClick={props.handleAddCustomProvider}
                    disabled={!props.newProviderUrl || !props.fetchedModels}
