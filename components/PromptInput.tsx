...
export const PromptInput: React.FC<PromptInputProps> = ({ onOptimize }) => {
  const { language, autoTranslate, setAutoTranslate } = useSettingsStore();
  const { prompt, setPrompt, isOptimizing, isTranslating } = useUIStore();
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !promptHistoryRef.current.contains(event.target as Node)
      ) {
